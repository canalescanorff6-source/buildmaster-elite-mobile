...
assert.equal(R432_GLOBAL_SOURCE_BUDGET_BYTES, 5.5 * 1024 * 1024);
assert.equal(R432_SOURCE_RESERVE_BYTES, 100_000);
assert.equal(R432_SOURCE_CHECKPOINT_BYTES, (5.5 * 1024 * 1024) - 100_000);
...
assert.match(read('scripts/check-bundle-budget.mjs'), /sourceTs: 5\.5 \* 1024 \* 1024/);
assert.match(read('scripts/apply-r407-scalable-vault-capacity.mjs'), /sourceBudget=5\.5\*1024\*1024/);
assert.match(read('tests/v40-80-r184-production-legacy-isolation-regression.mjs'), /sourceLimit=5\.5\*1024\*1024/);
assert.match(read('scripts/apply-r414-ci-contract-convergence.mjs'), /R414_SOURCE_BUDGET_BYTES = 5_667_168/);
assert.match(read('scripts/audit-r424-final-requirements-closure.mjs'), /5,5 MiB/);
assert.match(read('tests/v40-80-r424-final-requirements-closure-regression.mjs'), /sourceTs: 5\.5 \* 1024 \* 1024/);
...
console.log(`R432 aprovada: orçamento-fonte convergido em 5,5 MiB com reserva de ${R432_SOURCE_RESERVE_BYTES} bytes; margem para 5.412.980 bytes = ${newMargin}.`);
