...
assert.match(pipeline, /applyPerformanceEngine2027R70\(current\);\s*current = applyPerformanceEngine2027R107\(current\);\s*current = applyPerformanceEngine2027R108\(current\);\s*current = applyPerformanceEngine2027R109\(current\);\s*current = applyMasterCardEngineV4080R50\(current\);/);
...
assert.match(master, /if \(!positionR109\.applied\)/);
...
console.log('r107 compatibilidade aprovada: r109 > r108 > r107 > r70, mantendo GK corrigido.');
