...
assert.match(pipeline, /applyPerformanceEngine2027R70\(current\);\s*current = applyPerformanceEngine2027R107\(current\);\s*current = applyMasterCardEngineV4080R50\(current\);/);
...
assert.match(r100, /quality\?\.winner\.totalScore \?\? performance\?\.winner\.totalScore/);
...
console.log('r107 integração aprovada: Motor Mestre prefere Quality, fallback r70 e GK corrigido.');
