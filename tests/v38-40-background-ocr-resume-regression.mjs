...
const playBridge = read('scripts/install-play-store-bridge.mjs');
...
assert.match(installer, /ensurePermission\('FOREGROUND_SERVICE_DATA_SYNC'\)/);
assert.match(installer, /ensurePermission\('POST_NOTIFICATIONS'\)/);
assert.match(apkWorkflow, /install-background-ocr-plugin\.mjs/);
assert.match(apkWorkflow, /FOREGROUND_SERVICE_DATA_SYNC/);
assert.match(playWorkflow, /install-background-ocr-plugin\.mjs/);
assert.match(playWorkflow, /compileReleaseJavaWithJavac/);
assert.match(playBridge, /gradle\.replace\(\/dependencies/);
