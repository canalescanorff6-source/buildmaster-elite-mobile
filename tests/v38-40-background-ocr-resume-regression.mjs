...
const interactionR164 = read('src/modules/card-reader/readerInteractionRuntimeR164.ts');
...
assert.match(interactionR164, /analyzeSelectedImage\(restored, true\)/);
assert.match(app, /onClick=\{\(\) => void analyzeSelectedImage\(\)\}/);
assert.doesNotMatch(app, /onClick=\{analyzeSelectedImage\}/);
assert.match(analysisR163, /updateBackgroundOcrCheckpoint\(\{ stage: 'zones'/);
assert.match(analysisR163, /readEightEfhubCalibrationMacros\(activeFile, manualEfhubCalibration/);
