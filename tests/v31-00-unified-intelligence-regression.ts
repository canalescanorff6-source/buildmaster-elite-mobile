...
for (const result of [cf, amf, tallCf, agileCf]) {
  ...
  assert.match(result.unifiedIntelligence?.engineVersion ?? '', /^(?:(?:31\.00|31\.82)-unified-intelligence-|32\.00-unified-calibrated-)/);
  ...
  assert.match(result.buildName, /(?:Inteligência v31|Ficha v32)/);
}
...
assert.match(css, /BuildMaster v31\.00 — inteligência integrada/);
assert.match(css, /teste A\/B registrado por partida/);

console.log('v31.00 DNA 2.0, simulador profundo, integração, aprendizado e habilidades personalizadas aprovados.');
