...
assert.equal(APP_DATA_VERSION, '27.36.0');
assert.equal(APP_RELEASE_VERSION, '27.36.0');
assert.equal(compareVersions('27.1.0', '27.36.0'), -1);
assert.equal(compareVersions('27.36.0', '27.36.0'), 0);
assert.equal(compareVersions('26.99.9', '27.36.0'), -1);
...
const manifest = {
  ...
  channel: 'stable',
  updateType: 'apk',
  apkUrl: 'https://github.com/canalescanorff6-source/buildmaster-elite-mobile/releases/download/buildmaster-latest/BuildMaster-Elite-Tatico-v27.29.0-1352300020-abcdef12.apk',
...
assert.equal(evaluateUpdateManifest(manifest, { versionName: '27.36.0', versionCode: 1350900010 }, 'old-build').available, true);
assert.equal(evaluateUpdateManifest({ ...manifest, version: '27.36.0', versionCode: 1350900010, buildId: 'old-build' }, { versionName: '27.36.0', versionCode: 1350900010 }, 'old-build').available, false);
assert.equal(evaluateUpdateManifest({ ...manifest, appId: 'outro.app' }, { versionName: '27.36.0', versionCode: 1350900010 }, 'old-build').valid, false);
