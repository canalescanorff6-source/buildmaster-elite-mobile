...
assert.equal(APP_RELEASE_VERSION, '26.76.0');
assert.equal(compareVersions('26.77.0', '26.76.0'), 1);
assert.equal(compareVersions('26.76.0', '26.76.0'), 0);
assert.equal(compareVersions('26.75.9', '26.76.0'), -1);
...
const manifest = {
  ...
  channel: 'stable',
  updateType: 'apk',
  apkUrl: 'https://github.com/canalescanorff6-source/buildmaster-elite-mobile/releases/download/buildmaster-latest/BuildMaster-Elite-Tatico-latest.apk',
...
assert.equal(evaluateUpdateManifest(manifest, '26.76.0', 'old-build').available, true);
assert.equal(evaluateUpdateManifest({ ...manifest, version: '26.76.0', buildId: 'old-build' }, '26.76.0', 'old-build').available, false);
assert.equal(evaluateUpdateManifest({ ...manifest, appId: 'outro.app' }, '26.76.0', 'old-build').valid, false);
...
console.log('✓ Backup dedicado e Central de Atualizações v26.76 validados.');
