...
assert.equal(compareVersions('27.1.0', '27.20.0'), -1);
assert.equal(compareVersions('27.20.0', '27.20.0'), 0);
assert.equal(compareVersions('26.99.9', '27.20.0'), -1);
...
const manifest = {
  ...
  version: '27.20.0',
  versionCode: 1352000020,
  buildId: 'new-build',
  publishedAt: '2026-07-14T00:00:00.000Z',
  channel: 'stable',
  updateType: 'apk',
  apkUrl: 'https://github.com/canalescanorff6-source/buildmaster-elite-mobile/releases/download/buildmaster-latest/BuildMaster-Elite-Tatico-v27.20.0-1352000020-abcdef12.apk',
...
assert.equal(evaluateUpdateManifest(manifest, { versionName: '27.20.0', versionCode: 1350900010 }, 'old-build').available, true);
assert.equal(evaluateUpdateManifest({ ...manifest, version: '27.20.0', versionCode: 1350900010, buildId: 'old-build' }, { versionName: '27.20.0', versionCode: 1350900010 }, 'old-build').available, false);
assert.equal(evaluateUpdateManifest({ ...manifest, appId: 'outro.app' }, { versionName: '27.20.0', versionCode: 1350900010 }, 'old-build').valid, false);
...
console.log('✓ Backup e motor de atualização definitiva v27.20 validados.');
