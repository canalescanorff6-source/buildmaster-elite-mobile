...
assert.equal(compareVersions('27.1.0', '29.20.0'), -1);
assert.equal(compareVersions('29.20.0', '29.20.0'), 0);
assert.equal(compareVersions('26.99.9', '29.20.0'), -1);
...
const manifest = {
  ...
  version: '29.21.0',
  versionCode: 1_750_000_010,
  buildId: 'new-build',
  publishedAt: '2026-07-14T00:00:00.000Z',
  channel: 'stable',
  updateType: 'apk',
  apkUrl: 'https://github.com/canalescanorff6-source/buildmaster-elite-mobile/releases/download/buildmaster-latest/BuildMaster-Elite-Tatico-v29.21.0-1750000010-abcdef12.apk',
...
assert.equal(evaluateUpdateManifest(manifest, { versionName: '29.20.0', versionCode: 1_740_000_010 }, 'old-build').available, true);
assert.equal(evaluateUpdateManifest({ ...manifest, version: '28.80.0', versionCode: 1350900010, buildId: 'old-build' }, { versionName: '29.20.0', versionCode: 1_740_000_010 }, 'old-build').available, false);
assert.equal(evaluateUpdateManifest({ ...manifest, appId: 'outro.app' }, { versionName: '29.20.0', versionCode: 1_740_000_010 }, 'old-build').valid, false);
