...
assert.equal(APP_DATA_VERSION, '30.00.0');
assert.equal(APP_RELEASE_VERSION, '30.00.0');
assert.equal(compareVersions('27.1.0', '30.00.0'), -1);
assert.equal(compareVersions('30.00.0', '30.00.0'), 0);
assert.equal(compareVersions('26.99.9', '30.00.0'), -1);
...
const manifest = {
  ...
  version: '29.71.0',
  versionCode: 1_775_500_010,
  buildId: 'new-build',
  publishedAt: '2026-07-14T00:00:00.000Z',
  channel: 'stable',
  updateType: 'apk',
  apkUrl: 'https://github.com/canalescanorff6-source/buildmaster-elite-mobile/releases/download/buildmaster-latest/BuildMaster-Elite-Tatico-v29.71.0-1775500010-abcdef12.apk',
...
assert.equal(evaluateUpdateManifest(manifest, { versionName: '30.00.0', versionCode: 1_775_000_010 }, 'old-build').available, true);
assert.equal(evaluateUpdateManifest({ ...manifest, version: '28.80.0', versionCode: 1350900010, buildId: 'old-build' }, { versionName: '30.00.0', versionCode: 1_775_000_010 }, 'old-build').available, false);
assert.equal(evaluateUpdateManifest({ ...manifest, appId: 'outro.app' }, { versionName: '30.00.0', versionCode: 1_775_000_010 }, 'old-build').valid, false);
...
console.log('✓ Backup e motor de atualização definitiva v27.29 validados.');
