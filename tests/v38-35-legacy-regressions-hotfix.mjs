...
assert.ok(audit.includes("pkg.version === '38.36.0'"));
assert.ok(audit.includes('buildmaster-v38-36-deterministic-audit-1'));
assert.ok(doctor.includes('Regressões v38.36'));
assert.equal(manifest.name, 'BuildMaster Elite Tático v38.36');
assert.ok(sw.includes('buildmaster-v38-36-deterministic-audit-1'));
assert.ok(nativeCache.includes('38.36.0-deterministic-audit-1'));
const playNotes = read('play-store/listing/pt-BR/release-notes/38.36.0.txt').trim();
assert.ok(playNotes.length > 0 && playNotes.length <= 500);

console.log('v38.36 aprovada: regressões v31.82, v34.00 e v36.00 atualizadas sem reduzir as proteções do orçamento, visual ou auditoria.');
