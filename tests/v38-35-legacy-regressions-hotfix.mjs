...
assert.equal(pkg.version, '38.35.0');
...
assert.match(cacheRegression, /35\\.0-legacy-regressions/);
assert.match(cacheHotfixRegression, /35-legacy-regressions/);
assert.ok(audit.includes("pkg.version === '38.35.0'"));
assert.ok(audit.includes('buildmaster-v38-35-legacy-regressions-1'));
assert.ok(doctor.includes('Regressões v38.35'));
assert.equal(manifest.name, 'BuildMaster Elite Tático v38.35');
assert.ok(sw.includes('buildmaster-v38-35-legacy-regressions-1'));
assert.ok(nativeCache.includes('38.35.0-legacy-regressions-1'));
const playNotes = read('play-store/listing/pt-BR/release-notes/38.35.0.txt').trim();
assert.ok(playNotes.length > 0 && playNotes.length <= 500);

console.log('v38.35 aprovada: regressões v31.82, v34.00 e v36.00 atualizadas sem reduzir as proteções do orçamento, visual ou auditoria.');
