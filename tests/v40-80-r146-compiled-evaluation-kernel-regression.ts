...
for (const item of cases) {
  ...
  assert.equal(result.cleanSlate2027R119.guards.exactBudget,true,'v40-80-r146-compiled-evaluation-kernel-regression: orçamento atual precisa continuar exato.');
  assert.equal(result.recommendedSkills.length,5,'v40-80-r146-compiled-evaluation-kernel-regression: Top 5 atual precisa continuar completo.');
  assert.equal(new Set(result.recommendedSkills).size,5,'v40-80-r146-compiled-evaluation-kernel-regression: Top 5 atual não pode duplicar habilidades.');
  assert.ok(Number.isFinite(result.cleanSlate2027R119.score)&&result.cleanSlate2027R119.score>0,'v40-80-r146-compiled-evaluation-kernel-regression: score atual precisa permanecer válido.');
