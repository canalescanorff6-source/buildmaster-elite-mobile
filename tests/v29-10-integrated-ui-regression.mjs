...
pass(pkg.version === '30.00.0', 'Pacote final preserva a base v29.10 e avança para v30.00');
pass(pkg.scripts['test:all'].includes('npm run test:v2910'), 'Regressões v29.10 permanecem na bateria final');
...
pass(globals.includes('@import "./design-system-v2910-admin-update.css";') && globals.trim().endsWith('@import "./design-system-v3000-play-publication.css";'), 'Estilo v29.10 é preservado e v30.00 é a última camada visual');
pass(layout.includes('bm-v2910-admin-update'), 'Escopo visual v29.10 está ativo no layout');
pass(dataSafety.includes('CURRENT_DATA_SCHEMA = 3000') && dataSafety.includes("APP_DATA_VERSION = '30.00.0'"), 'Backup foi elevado sem remover seções anteriores');
