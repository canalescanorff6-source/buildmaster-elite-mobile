...
assert.notDeepEqual(sameCardCB.training,naturalCF.training,'Uma mudança real CF→CB precisa alterar a ficha, sem alterar a identidade da carta.');
assert.ok(sameCardCB.training.defending > naturalCF.training.defending,'Uso em CB precisa elevar retorno funcional da defesa.');
assert.ok(sameCardCB.training.shooting < naturalCF.training.shooting,'Uso em CB não deve preservar gasto ofensivo sem retorno funcional.');
