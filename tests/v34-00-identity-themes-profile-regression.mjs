...
const settingsWorkspaceR190 = read('src/components/settings/CardVisionSettingsWorkspaceR190.tsx');
...
const experienceControllerR178 = read('src/modules/experience/cardVisionExperienceControllerR178.ts');
...
assert.match(settingsWorkspaceR190, /<IdentityAppearancePanel/, 'Aparência deve continuar integrada pela fronteira R190 de Ajustes.');
assert.match(appearancePanel, /<ProfileAvatarEditor/);
assert.doesNotMatch(app, /saveProfileAvatar|removeProfileAvatar/, 'Persistência de avatar não deve voltar ao shell.');
assert.match(experienceControllerR178, /import\('@\/lib\/profileAvatar'\)/, 'Controller deve adquirir a autoridade de avatar sob demanda.');
assert.match(experienceControllerR178, /saveProfileAvatar\(next\)/, 'Salvar avatar deve continuar usando profileAvatar.ts.');
assert.match(experienceControllerR178, /removeProfileAvatar\(\)/, 'Remover avatar deve continuar usando profileAvatar.ts.');
