...
expect(playPreflight.includes('release-notes/${packageVersion}.txt') || playPreflight.includes(`release-notes/${currentVersion}.txt`), 'Pré-voo Play ainda usa notas de uma versão anterior.');
...
expect(panel.includes("if (!result?.success || !result.userId") && panel.includes('result.authConfirmed') && panel.includes('result.profileConfirmed') && panel.includes('result.expiryConfirmed'), 'Cliente aceita resposta sem confirmação completa do usuário criado.');
...
expect(edge.includes("return respond({ success: true, userId: created.user.id, username, requestId") && edge.includes('authConfirmed: true') && edge.includes('profileConfirmed: true') && edge.includes('expiryConfirmed: true'), 'Servidor não confirma criação completa com usuário e referência.');
