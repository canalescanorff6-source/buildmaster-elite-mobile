...
expect(preflight.includes("version === '31.74.0'"), 'Pré-voo de produção ainda valida uma versão antiga.');
expect(preflight.includes('BuildMaster Elite Tático v31.74'), 'Pré-voo não valida o manifesto PWA da v31.74.');
expect(preflight.includes('npm run test:v3174'), 'Pré-voo não valida a bateria atual da v31.74.');
expect(ciDoctor.includes("['Regressões v31.74', ['run', 'test:v3174']]"), 'Diagnóstico consolidado não executa a regressão v31.74.');
expect(manifest.name === 'BuildMaster Elite Tático v31.74' && manifest.short_name === 'BuildMaster v31.74', 'Manifesto PWA não está na v31.74.');
expect(serviceWorker.includes('buildmaster-v31-74'), 'Cache PWA não foi renovado para v31.74.');
expect(playPreflight.includes('release-notes/31.74.0.txt'), 'Pré-voo Play ainda usa notas de uma versão anterior.');
