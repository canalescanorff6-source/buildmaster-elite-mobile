...
for (const result of [scholesMlgA, scholesMlgB, scholesMat, ronaldoSa]) {
  ...
  assert.match(result.buildName, /(?:Motor Adaptativo por Carta v39\.30 \+ Função Real v39\.40|Ficha Automática v\d+\.\d{2}(?: r\d+)? — (?:Precisão Competitiva 99|Desempenho Máximo Stack Final|eFootball 2027 v6\.0 Adaptativa|Desempenho Real 2027))/);
}
