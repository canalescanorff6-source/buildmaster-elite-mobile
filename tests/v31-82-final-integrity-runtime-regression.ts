...
assert.equal((result as typeof result & { cleanSlate2027R119?: { currentImpeto?: string | null; impetoDecision?: string } }).cleanSlate2027R119?.currentImpeto?.toLowerCase(), 'chute', 'O r119 deve reconhecer o Ímpeto já aplicado como recurso atual da carta.');
assert.equal((result as typeof result & { cleanSlate2027R119?: { currentImpeto?: string | null; impetoDecision?: string } }).cleanSlate2027R119?.impetoDecision, 'KEEP_CURRENT', 'Ímpeto já aplicado deve ser preservado, não recomendado novamente.');
