...
assert.match(result.localAi?.engineVersion ?? '', /^30\.40-local-ai-/);
...
assert.match(css, /BuildMaster v30\.40 — IA local/);
assert.match(css, /\.bm-ai-impeto-winner/);

console.log('v30.40 IA local e recomendação visível de Ímpetos aprovadas.');
