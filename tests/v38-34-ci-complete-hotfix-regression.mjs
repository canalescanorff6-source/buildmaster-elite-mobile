...
assert.ok(audit.includes("pkg.version === '38.34.0'"));
assert.ok(audit.includes("buildmaster-v38-34-ci-stability-1"));
assert.ok(doctor.includes('Regressões v38.34'));
assert.ok(sw.includes('buildmaster-v38-34-ci-stability-1'));
assert.ok(nativeCache.includes('38.34.0-ci-stability-1'));

console.log('v38.34 aprovado: orçamento protegido, rota inicial restaurada e contrato do gerador compatível com o Estúdio Marques.');
