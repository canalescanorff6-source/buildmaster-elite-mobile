...
assert.equal(pkg.version, '27.25.0', 'package.json precisa estar na v27.25.0');
...
assert.ok(layout.includes('v27.25'), 'Metadados da interface não correspondem à v27.25.');
assert.ok(manifest.includes('v27.25'), 'manifest.webmanifest não corresponde à v27.25.');
assert.ok(sw.includes('27-25'), 'Cache do service worker não corresponde à v27.25.');
...
console.log('release integrity v27.25: assinatura, APK único, manifesto e verificação pós-publicação alinhados.');
