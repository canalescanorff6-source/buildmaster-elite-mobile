...
assert.equal(pkg.version, '27.21.0', 'package.json precisa estar na v27.21.0');
...
assert.ok(layout.includes('v27.21'), 'Metadados da interface não correspondem à v27.21.');
assert.ok(manifest.includes('v27.21'), 'manifest.webmanifest não corresponde à v27.21.');
assert.ok(sw.includes('27-21'), 'Cache do service worker não corresponde à v27.21.');
...
assert.ok(workflow.includes('Publicar pacote e manifesto imutáveis'), 'APK e manifesto imutáveis precisam ser publicados juntos na release isolada.');
assert.ok(workflow.includes('Verificar release imutável antes de ativar'), 'A release precisa ser baixada e conferida antes de virar latest.');
assert.ok(workflow.includes('Verificar canal novo, API e compatibilidade antiga'), 'Os dois canais precisam ser verificados após a publicação.');
...
console.log('release integrity v27.21: assinatura, APK único, manifesto e verificação pós-publicação alinhados.');
