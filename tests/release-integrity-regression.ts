...
assert.equal(pkg.version, '25.74.0');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
for (const content of [workflow, layout, manifest]) assert.match(content, /25\.74/);
assert.match(sw, /25-74/);
assert.doesNotMatch(workflow, /v25\.72/);
