...
assert.equal(pkg.version, '27.20.0', 'package.json precisa estar na v27.20.0');
...
assert.ok(layout.includes('v27.20'), 'Metadados da interface não correspondem à v27.20.');
assert.ok(manifest.includes('v27.20'), 'manifest.webmanifest não corresponde à v27.20.');
assert.ok(sw.includes('27-20'), 'Cache do service worker não corresponde à v27.20.');
...
assert.ok(workflow.includes('manifesto por último'), 'Manifesto precisa ser publicado depois do APK.');
assert.ok(workflow.includes('Verificar release publicada de ponta a ponta'), 'A release precisa ser baixada e conferida após a publicação.');
...
console.log('release integrity v27.20: assinatura, APK único, manifesto e verificação pós-publicação alinhados.');
