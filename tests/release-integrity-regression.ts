...
assert.equal(pkg.version, '27.24.0', 'package.json precisa estar na v27.24.0');
...
assert.ok(layout.includes('v27.24'), 'Metadados da interface não correspondem à v27.24.');
assert.ok(manifest.includes('v27.24'), 'manifest.webmanifest não corresponde à v27.24.');
assert.ok(sw.includes('27-24'), 'Cache do service worker não corresponde à v27.24.');
...
console.log('release integrity v27.24: assinatura, APK único, manifesto e verificação pós-publicação alinhados.');
