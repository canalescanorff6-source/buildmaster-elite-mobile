...
assert.equal(pkg.version, '27.23.0', 'package.json precisa estar na v27.23.0');
...
assert.ok(layout.includes('v27.23'), 'Metadados da interface não correspondem à v27.23.');
assert.ok(manifest.includes('v27.23'), 'manifest.webmanifest não corresponde à v27.23.');
assert.ok(sw.includes('27-23'), 'Cache do service worker não corresponde à v27.23.');
...
console.log('release integrity v27.23: assinatura, APK único, manifesto e verificação pós-publicação alinhados.');
