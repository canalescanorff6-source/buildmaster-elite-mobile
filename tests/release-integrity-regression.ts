...
assert.equal(pkg.version, '27.26.0', 'package.json precisa estar na v27.26.0');
...
assert.ok(layout.includes('v27.26'), 'Metadados da interface não correspondem à v27.26.');
assert.ok(manifest.includes('v27.26'), 'manifest.webmanifest não corresponde à v27.26.');
assert.ok(sw.includes('27-26'), 'Cache do service worker não corresponde à v27.26.');
...
console.log('release integrity v27.26: assinatura, APK único, manifesto e verificação pós-publicação alinhados.');
