...
assert.deepEqual(defaults.map((zone) => zone.shortLabel), [
  ...
  'Boosters / ímpeto', '26 atributos', 'Modelo físico', 'Habilidades'
]);
...
assert.equal(ocr.length, 19, 'O mapa manual deve gerar as 19 subáreas internas do OCR.');
...
assert.ok(restored);
assert.equal(restored?.zones.length, 8);
...
void (async () => {
  ...
  console.log('v31.81: calibrador visual com oito áreas proporcionais e recortes internos precisos aprovado.');
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
