...
assert.match(workflow, /Validar canal principal e o APK apontado|Validar ponte legacy completa/);
...
assert.match(panel, /routeIndex < routeCandidates\.length|routeIndex < manifests\.length/);
assert.match(panel, /Testando rota oficial|Rota automática/);
