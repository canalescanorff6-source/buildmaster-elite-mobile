...
import { CLEAN_SLATE_SEARCH_OPTIMIZATION_R144_VERSION } from '../src/lib/cleanSlatePerformance2027V4080R119';
...
for (const item of cases) {
  ...
  assert.equal(r144.version,CLEAN_SLATE_SEARCH_OPTIMIZATION_R144_VERSION);
  assert.equal(r144.beamWidth,20);
  assert.equal(r144.heuristicChanged,false);
  assert.equal(r144.beamReduced,false,'Ganho R144 não pode vir de reduzir o beam.');
  assert.equal(r144.earlyDuplicateSuppression,true);
  assert.equal(r144.orderedBeamInsertion,true);
  assert.equal(r144.sharedDiagnosticMemo,true);
  assert.ok(r144.duplicateStatesSkipped>0,'Casos reais devem demonstrar supressão antecipada de estados equivalentes.');
  assert.equal(r144.generatedStates,r144.uniqueFrontierStates+r144.duplicateStatesSkipped);
  assert.equal(r144.uniqueFrontierStates,r143.uniqueEvaluations,'A fronteira única R144 deve ser exatamente o conjunto que R143 avaliava uma vez.');
  assert.equal(r144.orderedBeamInsertions+r144.orderedBeamRejections,r144.uniqueFrontierStates-1,'Todo estado único além do zero deve ser inserido ou rejeitado deterministicamente.');
  assert.ok(r144.diagnosticUniqueEvaluations>0,'Saturação/retorno marginal/lab devem continuar sendo auditados.');
  assert.ok(r144.diagnosticCacheHits>0,'Diagnósticos devem reutilizar avaliações equivalentes.');
}

console.log('r144 aprovada: deduplicação antes da avaliação, beam ordenado e memoização de diagnósticos reduzem CPU mantendo beam 20, heurística e saídas R143 idênticas.');
