...
assert.ok(pkg.scripts['test:all'].startsWith('npm run test:v3000 && npm run test:v2980 && npm run test:v2970 && npm run test:v2960 && npm run test:v2950 && npm run test:v2940 && npm run test:v2930') && pkg.scripts['test:all'].includes('npm run test:v2800'));
...
assert.match(globals.trim(), /@import "\.\/design-system-v3000-play-publication\.css";$/);
...
assert.equal(manifest.short_name, 'BuildMaster v30.00');
