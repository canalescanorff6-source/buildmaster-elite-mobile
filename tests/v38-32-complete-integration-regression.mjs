...
assert.equal(tactical.META_FORMATION_STUDIO_VERSION, '40.00.0');
...
for (const marker of ['40.00.0', 'commandEvidence', 'candidate', 'confirmed', 'repeated', 'annotations', 'offensive-transition', 'defensive-transition', 'replaceMatchTrainerSessions']) assert.ok(videoEngine.includes(marker), `motor de vídeo sem ${marker}`);
...
assert.equal(lock.version, '40.00.0');
assert.equal(lock.packages[''].version, '40.00.0');
assert.equal(JSON.parse(read('public/manifest.webmanifest')).name, 'BuildMaster Elite Tático v40.00');
