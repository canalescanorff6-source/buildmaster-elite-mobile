...
assert.equal(tactical.META_FORMATION_STUDIO_VERSION, '38.32.0');
...
for (const marker of ['38.32.0', 'commandEvidence', 'candidate', 'confirmed', 'repeated', 'annotations', 'offensive-transition', 'defensive-transition', 'replaceMatchTrainerSessions']) assert.ok(videoEngine.includes(marker), `motor de vídeo sem ${marker}`);
...
assert.equal(pkg.version, '38.32.0');
assert.equal(lock.version, '38.32.0');
assert.equal(lock.packages[''].version, '38.32.0');
assert.equal(JSON.parse(read('public/manifest.webmanifest')).name, 'BuildMaster Elite Tático v38.32');
assert.ok(read('public/sw.js').includes('buildmaster-v38-32-complete-integration-1'));
assert.ok(read('src/components/RegisterServiceWorker.tsx').includes('38.32.0-complete-integration-1'));
