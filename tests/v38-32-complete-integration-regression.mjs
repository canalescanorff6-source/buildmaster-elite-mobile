...
for (const marker of ['38.33.0', 'commandEvidence', 'candidate', 'confirmed', 'repeated', 'annotations', 'offensive-transition', 'defensive-transition', 'replaceMatchTrainerSessions']) assert.ok(videoEngine.includes(marker), `motor de vídeo sem ${marker}`);
...
assert.equal(lock.version, '38.33.0');
assert.equal(lock.packages[''].version, '38.33.0');
assert.equal(JSON.parse(read('public/manifest.webmanifest')).name, 'BuildMaster Elite Tático v38.33');
assert.ok(read('public/sw.js').includes('buildmaster-v38-33-professional-template-1'));
assert.ok(read('src/components/RegisterServiceWorker.tsx').includes('38.33.0-professional-template-1'));
