...
const apkUrl = 'https://github.com/canalescanorff6-source/buildmaster-elite-mobile/releases/download/buildmaster-latest/BuildMaster-Elite-Tatico-v27.28.0-1352300042-abcdef12.apk';
assert.equal(isTrustedApkUrl(apkUrl), true);
const immutableUrl = 'https://github.com/canalescanorff6-source/buildmaster-elite-mobile/releases/download/buildmaster-v27.28.0-1352300042-01/BuildMaster-Elite-Tatico-v27.28.0-135230004201-abcdef12.apk';
...
assert.equal(isTrustedApkUrl('https://github.com/canalescanorff6-source/buildmaster-elite-mobile/releases/download/outra-tag/BuildMaster-Elite-Tatico-v27.28.0-1352300042-abcdef12.apk'), false);
...
assert.equal(evaluateUpdateManifest(manifest, { versionName: '27.28.0', versionCode: 1350900010 }, 'old').available, true);
assert.equal(evaluateUpdateManifest({ ...manifest, version: '27.28.0', versionCode: 1350900010 }, { versionName: '27.28.0', versionCode: 1350900010 }, 'abcdef123456').available, false);
assert.equal(evaluateUpdateManifest({ ...manifest, versionCode: 1350809999 }, { versionName: '27.28.0', versionCode: 1350900010 }, 'old').available, false);
...
console.log('✓ v27.28: detecção, versionCode, APK único, progresso, permissão, SHA-256, pacote e assinatura validados.');
