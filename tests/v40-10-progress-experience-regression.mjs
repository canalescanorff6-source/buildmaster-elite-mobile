...
assert.ok(register.includes('40.10.0-progress-runtime-1'));
...
assert.ok(layout.includes('bm-v4010-progress'));
...
assert.ok(updater.includes('v4010-global-update-progress'));
...
assert.ok(css.includes('.v4010-progress-track'));
assert.ok(css.includes('.v4010-global-update-progress'));
assert.ok(css.includes('.v4010-reader-progress'));
assert.ok(css.includes('.update-download-progress{display:none!important}'));

assert.ok(String(pkg.scripts?.['test:all']).endsWith('npm run test:v4010'));
console.log('v40.10 aprovada: download/validação/instalador e leitura de cartas possuem progresso visual, porcentagem e estimativa sem regressão do leitor v40.00.');
