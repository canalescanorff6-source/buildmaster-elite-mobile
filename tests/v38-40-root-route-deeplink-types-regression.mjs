...
const navigationR176 = fs.readFileSync('src/hooks/useCardVisionNavigationControllerR176.ts', 'utf8');
...
assert.match(navigationR176, /const targetWorkspace = deepLink\.workspace \?\? 'visao-geral'/, 'Deep link precisa normalizar workspace opcional.');
assert.match(navigationR176, /if \(deepLink\.group === 'jogadores'\) setPlayerWorkspace\(targetWorkspace\)/, 'Estado de jogadores só pode receber PlayerWorkspace definido.');
assert.doesNotMatch(navigationR176, /setPlayerWorkspace\(deepLink\.workspace\)/, 'Workspace opcional não pode ser enviado diretamente ao setter.');
