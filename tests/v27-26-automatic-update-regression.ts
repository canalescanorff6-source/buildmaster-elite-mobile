...
assert.match(pkg.scripts['test:all'], /^npm run test:v2726 &&/);
...
const newest: UpdateManifestCandidate = {
  payload: {},
  manifest: manifest('27.26.0', 1_376_000_011, 'buildmaster-v27.26.0-1376000011-01'),
...
assert.ok(workflow.includes('$LEGACY_FINAL_APK_URL?finalVerify='), 'a validação pública do APK legado deve iniciar a query string com ?');
assert.ok(!workflow.includes('$LEGACY_FINAL_APK_URL&finalVerify='), 'a URL imutável do APK não pode usar & sem uma query anterior');
...
assert.match(nativeInstaller, /BuildMaster-Elite-Tatico-Updater\/27\.26 Android/);
...
console.log('✓ v27.26: seleção do maior versionCode, três canais, cópia local e abertura automática do instalador aprovados.');
