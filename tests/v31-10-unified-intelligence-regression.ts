...
for (const result of [cf, amf, tallCf, agileCf]) {
  ...
  assert.match(result.unifiedIntelligence?.engineVersion ?? '', /^(?:(?:31\.10|31\.30|31\.82)-unified-intelligence-|32\.00-unified-calibrated-)/);
  ...
  assert.match(result.buildName, /(?:Inteligência v31|Ficha Elite Suprema|Ficha v32)/);
}
