...
assert.match(workflow, /Publicar manifesto do canal após validar o APK imutável/);
assert.ok(
  workflow.indexOf('Publicar APK versionado na ponte fixa') < workflow.indexOf('Publicar manifesto do canal após validar o APK imutável'),
  'O APK versionado precisa existir e ser validado antes do manifesto principal.'
);
assert.match(workflow, /BuildMaster-Elite-Tatico-Updater\/29\.10 Android/);
