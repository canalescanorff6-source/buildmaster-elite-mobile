...
assert.equal(pkg.version, '31.10.0');
assert.equal(lock.version, '31.10.0');
assert.equal(lock.packages[''].version, '31.10.0');
assert.equal(pkg.scripts['test:all'], 'npm run test:v3000 && npm run test:v3010 && npm run test:v3020 && npm run test:v3030 && npm run test:v3040 && npm run test:v3050 && npm run test:v3100 && npm run test:v3110 && npm run quality:audit');
assert.match(read('src/lib/dataSafety.ts'), /CURRENT_DATA_SCHEMA = 3000/);
assert.match(read('src/lib/dataSafety.ts'), /APP_DATA_VERSION = '31\.10\.0'/);
...
assert.equal(JSON.parse(read('public/manifest.webmanifest')).name, 'BuildMaster Elite Tático v31.10');

console.log('v31.10 integração de produção limpa aprovada.');
