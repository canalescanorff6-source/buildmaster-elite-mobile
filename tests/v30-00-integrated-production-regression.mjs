...
assert.equal(pkg.version, '31.73.0');
assert.equal(lock.version, '31.73.0');
assert.equal(lock.packages[''].version, '31.73.0');
...
assert.match(read('src/lib/dataSafety.ts'), /APP_DATA_VERSION = '31\.73\.0'/);
...
assert.equal(JSON.parse(read('public/manifest.webmanifest')).name, 'BuildMaster Elite Tático v31.73');

console.log('v31.73 integração de produção limpa aprovada.');
