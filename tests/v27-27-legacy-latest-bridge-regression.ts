...
assert.match(pkg.scripts['test:all'], /^npm run test:v2727 &&/);
assert.equal(
  DEFAULT_UPDATE_MANIFEST_URL,
...
assert.equal(isTrustedApkUrl(apkUrl.replace('/buildmaster-latest/', '/buildmaster-v27.27.0-1377000001-01/')), true);
...
const publishApk = workflow.indexOf('Publicar BuildMaster-Elite-Tatico-latest.apk na ponte antiga');
const verifyApk = workflow.indexOf('Validar publicamente o APK latest antes do manifesto');
const publishManifest = workflow.indexOf('Ativar manifesto automático por último');
assert.ok(publishApk > 0 && verifyApk > publishApk && publishManifest > verifyApk);

console.log('✓ v27.27: BuildMaster-Elite-Tatico-latest.apk publicado, validado e ativado antes do manifesto da v27.00.');
