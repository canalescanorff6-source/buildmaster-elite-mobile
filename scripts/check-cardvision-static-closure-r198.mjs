...
const MAX_MODULES_R198 = 178;
const MAX_SOURCE_BYTES_R198 = 2_377_300;
...
for (const [relative, label] of [
  ...
  if (seen.has(path.resolve(path.join(srcRoot, relative)))) throw new Error(`R198: ${label} voltou à árvore estática inicial.`);
}
if (seen.size > MAX_MODULES_R198) throw new Error(`R198: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R198}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R198) throw new Error(`R198: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R198}).`);
console.log(`R198 static closure aprovada: ${seen.size} módulos, ${sourceBytes} bytes; boundaries R188/R190/R191 seguem lazy.`);
