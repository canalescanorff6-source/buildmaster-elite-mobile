...
export function applyReviewedR119BaselinesR186R200(rootDirectory=process.cwd()){
  ...
  if(actual!==REVIEWED_SHA&&!downstreamR416&&!downstreamR417&&!downstreamR452) throw new Error(`R406-fix7: SHA R119 não revisado: ${actual}; esperados ${REVIEWED_SHA}, ${R416_SHA} ou runtime sucessor R417/R452 reconhecido.`);
  const expected=(downstreamR452||downstreamR417)?actual:downstreamR416?R416_SHA:REVIEWED_SHA;
  const results=[];
  ...
  return {changed:results.some(x=>x.changed),sourceSha:actual,expectedSha:expected,downstreamR416,downstreamR417,downstreamR452,results};
}
