...
if (sourceBuildMatch) {
  check(
    /^v40\.00-ci-[a-z0-9-]+-\d{8}-r\d+$/.test(sourceBuildMatch[1]),
    `CI_SOURCE_BUILD inválido: ${sourceBuildMatch[1]}.`,
  );
}
check(doctor.includes('EXPECTED_FULL_GROUPS = 85'), 'ci-doctor não protege a quantidade esperada de 85 grupos.');
...
for (const [version, script] of [['39.30', 'test:v3930'], ['39.40', 'test:v3940'], ['39.50', 'test:v3950'], ['40.00', 'test:v4000']]) {
