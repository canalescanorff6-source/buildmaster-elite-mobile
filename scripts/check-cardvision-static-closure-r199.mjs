...
const MAX_MODULES_R199 = 178;
const MAX_SOURCE_BYTES_R199 = 2_377_220;
...
for (const [relative, label] of [
  ...
  if (seen.has(path.resolve(path.join(srcRoot, relative)))) throw new Error(`R199: ${label} voltou à árvore estática inicial.`);
}
if (seen.size > MAX_MODULES_R199) throw new Error(`R199: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R199}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R199) throw new Error(`R199: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R199}).`);
console.log(`R199 static closure aprovada: ${seen.size} módulos, ${sourceBytes} bytes; boundaries R188/R190/R191 seguem lazy.`);
