...
export const R432_SOURCE_BUDGET_CONVERGENCE_VERSION = '40.80-r432-source-budget-convergence-v1';
export const R432_GLOBAL_SOURCE_BUDGET_BYTES = 5.5 * 1024 * 1024;
...
function patchBundle(source) {
  return replaceAllKnown(source, [
    ['sourceTs: 5.25 * 1024 * 1024', 'sourceTs: 5.5 * 1024 * 1024'],
  ]);
}

function patchR407(source) {
  return replaceAllKnown(source, [
    ['const sourceBudget=5.25*1024*1024;', 'const sourceBudget=5.5*1024*1024;'],
  ]);
}

function patchR184(source) {
  return replaceAllKnown(source, [
    ['const sourceLimit=5.25*1024*1024;', 'const sourceLimit=5.5*1024*1024;'],
  ]);
}

function patchR414(source) {
  return replaceAllKnown(source, [
    ['R414_SOURCE_BUDGET_BYTES = 5_405_024', 'R414_SOURCE_BUDGET_BYTES = 5_667_168'],
    ['5,25 MiB', '5,5 MiB'],
    ["sourceTs: 5.25 * 1024 * 1024", "sourceTs: 5.5 * 1024 * 1024"],
    ['5\\.25\\s*\\*\\s*1024', '5\\.5\\s*\\*\\s*1024'],
    ['sourceGlobalLimitBytes: 5.25 * 1024 * 1024', 'sourceGlobalLimitBytes: 5.5 * 1024 * 1024'],
  ]);
}

function patchR424Audit(source) {
  return replaceAllKnown(source, [
    ['5.25 * 1024 * 1024', '5.5 * 1024 * 1024'],
    ['5\\.25\\s*\\*\\s*1024', '5\\.5\\s*\\*\\s*1024'],
    ['5_405_024', '5_667_168'],
    ['5,25 MiB', '5,5 MiB'],
  ]);
...
export function auditR432SourceBudgetConvergence(rootDirectory = process.cwd()) {
  ...
  if (!content.bundle.includes('sourceTs: 5.5 * 1024 * 1024')) issues.push(`${TARGETS.bundle}: teto global não está em 5,5 MiB.`);
  if (!content.r407.includes('const sourceBudget=5.5*1024*1024;')) issues.push(`${TARGETS.r407}: R407 não usa 5,5 MiB.`);
  if (!content.r184.includes('const sourceLimit=5.5*1024*1024;')) issues.push(`${TARGETS.r184}: R184 não usa 5,5 MiB.`);
  ...
  if (!content.r414.includes('R414_SOURCE_BUDGET_BYTES = 5_667_168')) issues.push(`${TARGETS.r414}: checkpoint R414 não está em 5.667.168 bytes.`);
  if (!content.r414.includes('sourceTs: 5.5 * 1024 * 1024')) issues.push(`${TARGETS.r414}: guardrail global R414 ainda diverge.`);
  if (!content.r424Audit.includes('5_667_168') || !content.r424Audit.includes('5,5 MiB')) issues.push(`${TARGETS.r424Audit}: auditoria R424 ainda valida o baseline antigo.`);
  if (!content.r424Fixture.includes('sourceTs: 5.5 * 1024 * 1024') || !content.r424Fixture.includes('R414_SOURCE_BUDGET_BYTES = 5_667_168')) issues.push(`${TARGETS.r424Fixture}: fixture R424 ainda representa 5,25 MiB.`);
...
if (invoked === import.meta.url) {
  ...
  console.log(result.changed
    ? `R432 convergiu ${result.patched.length} contrato(s) de orçamento-fonte em 5,5 MiB com reserva de 100 KB.`
