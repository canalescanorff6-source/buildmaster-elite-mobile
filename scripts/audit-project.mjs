...
check(pkg.version === '31.72.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'31.72.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '31.72.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3000'), 'Esquema de dados sincronizado');
check(manifest.name === 'BuildMaster Elite Tático v31.72', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v31-72'), 'Cache PWA sincronizado');
...
check(pkg.scripts?.['test:all'] === 'npm run test:v3000 && npm run test:v3010 && npm run test:v3020 && npm run test:v3030 && npm run test:v3040 && npm run test:v3050 && npm run test:v3100 && npm run test:v3110 && npm run test:v3120 && npm run test:v3130 && npm run test:v3140 && npm run test:v3150 && npm run test:v3160 && npm run test:v3170 && npm run test:v3171 && npm run test:v3172 && npm run quality:audit', 'Bateria de testes limpa e atual');
