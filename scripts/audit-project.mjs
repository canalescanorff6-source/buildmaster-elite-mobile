...
check(pkg.version === '40.00.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'40.00.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '40.00.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3100'), 'Versão de dados sincronizada e esquema compatível');
check(manifest.name === 'BuildMaster Elite Tático v40.00', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v40-00-card-reader-rebuild-1'), 'Cache PWA sincronizado');
for (const file of [
