...
check(pkg.version === '33.00.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'33.00.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '33.00.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3000'), 'Esquema de dados sincronizado');
check(manifest.name === 'BuildMaster Elite Tático v33.00', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v33-00'), 'Cache PWA sincronizado');
...
check(cssFiles.some((file) => path.basename(file) === 'globals.css') && cssFiles.some((file) => path.basename(file) === 'v33-executive.css'), 'Tema base e camada Executive presentes', cssFiles.map((file) => path.relative(root, file)).join(', '));
check(!/@import\s+['"]/i.test(globals) && !/@import\s+['"]/i.test(executiveCss), 'CSS sem cadeia antiga de imports');
...
check(String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3200') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3300') && String(pkg.scripts?.['test:all'] ?? '').endsWith('npm run quality:audit'), 'Bateria de testes limpa e atual');
