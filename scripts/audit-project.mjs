...
check(pkg.version === '38.39.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'38.39.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '38.39.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3100'), 'Versão de dados sincronizada e esquema compatível');
check(manifest.name === 'BuildMaster Elite Tático v38.39', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v38-39-legacy-contract-final-1'), 'Cache PWA sincronizado');
