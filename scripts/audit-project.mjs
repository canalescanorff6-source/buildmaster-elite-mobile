...
check(pkg.version === '31.79.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'31.79.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '31.79.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3000'), 'Esquema de dados sincronizado');
check(manifest.name === 'BuildMaster Elite Tático v31.79', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v31-79'), 'Cache PWA sincronizado');
