...
check(pkg.version === '40.80.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'40.80.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '40.80.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3100'), 'Versão de dados sincronizada e esquema compatível');
check(manifest.name === 'BuildMaster Elite Tático v40.80', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v40-80-edge-stack-1'), 'Cache PWA sincronizado');
for (const file of [
...
check(String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3200') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3300') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3400') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3500') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3510') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3520') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3600') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3700') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run quality:audit') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v4020') && String(pkg.scripts?.['test:all'] ?? '').endsWith('npm run test:v4080'), 'Bateria de testes limpa e atual');
check((workflowApk.includes('npm run test:all') || workflowApk.includes('npm run ci:verify') || workflowApk.includes('npm run ci:diagnose-all')) && (workflowPlay.includes('npm run test:all') || workflowPlay.includes('npm run ci:verify') || workflowPlay.includes('npm run ci:diagnose-all')), 'Workflows usam a bateria atual diretamente ou pelo diagnóstico consolidado');
...
console.log('Auditoria do projeto limpo aprovada.');check(exists('src/lib/maximumPerformanceV4080.ts') && exists('src/app/v40-edge-safe.css') && String(pkg.scripts?.['test:v4080'] ?? '').includes('v40-80-maximum-stack-runtime-regression.ts'), 'v40.80: stack final e safe-area integrados');
