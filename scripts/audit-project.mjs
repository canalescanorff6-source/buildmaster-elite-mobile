...
check(pkg.version === '30.50.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'30.50.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '30.50.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3000'), 'Esquema de dados sincronizado');
check(manifest.name === 'BuildMaster Elite Tático v30.50', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v30-50'), 'Cache PWA sincronizado');
...
check(pkg.scripts?.['test:all'] === 'npm run test:v3000 && npm run test:v3010 && npm run test:v3020 && npm run test:v3030 && npm run test:v3040 && npm run test:v3050 && npm run quality:audit', 'Bateria de testes limpa e atual');
