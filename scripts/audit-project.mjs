...
check(pkg.version === '34.00.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'34.00.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '34.00.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3000'), 'Esquema de dados sincronizado');
check(manifest.name === 'BuildMaster Elite Tático v34.00', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v34-00'), 'Cache PWA sincronizado');
...
check(String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3200') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3300') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3400') && String(pkg.scripts?.['test:all'] ?? '').endsWith('npm run quality:audit'), 'Bateria de testes limpa e atual');
