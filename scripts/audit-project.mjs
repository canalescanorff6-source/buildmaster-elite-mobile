...
check(pkg.version === '40.20.0', 'Versão atual configurada', pkg.version);
check(appUpdates.includes("'40.20.0'"), 'Motor de atualização sincronizado');
check(dataSafety.includes("APP_DATA_VERSION = '40.20.0'") && dataSafety.includes('CURRENT_DATA_SCHEMA = 3100'), 'Versão de dados sincronizada e esquema compatível');
check(manifest.name === 'BuildMaster Elite Tático v40.20', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v40-20-progress-1'), 'Cache PWA sincronizado');
for (const file of [
