...
const MAX_MODULES_R174 = 191;
const MAX_SOURCE_BYTES_R174 = 2_713_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R174: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R174: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R174) throw new Error(`R174: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R174}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R174) throw new Error(`R174: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R174}).`);
console.log(`R174 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
