...
const MAX_MODULES_R175 = 191;
const MAX_SOURCE_BYTES_R175 = 2_706_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R175: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R175: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R175) throw new Error(`R175: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R175}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R175) throw new Error(`R175: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R175}).`);
console.log(`R175 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
