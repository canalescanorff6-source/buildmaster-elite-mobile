...
const hasR186AnalyzerBoundaries = fs.existsSync(path.join(root, 'src/modules/analysis/analyzerCardEvidenceR186.ts')) && fs.existsSync(path.join(root, 'src/modules/analysis/analyzerSkillIntelligenceR186.ts'));
const MAX_MODULES_R185 = hasR186AnalyzerBoundaries ? 182 : 180;
const MAX_SOURCE_BYTES_R185 = 2_445_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R185: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R185: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R185) throw new Error(`R185: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R185}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R185) throw new Error(`R185: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R185}).`);
console.log(`R185 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
