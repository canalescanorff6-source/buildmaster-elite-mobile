...
const MAX_MODULES_R155 = 265;
const MAX_SOURCE_BYTES_R155 = 3_900_000;
...
if (seen.size > MAX_MODULES_R155) {
  throw new Error(`R155: árvore estática do CardVision cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R155}).`);
}
if (sourceBytes > MAX_SOURCE_BYTES_R155) {
  throw new Error(`R155: árvore estática do CardVision cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R155}).`);
}

console.log(`R155 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
