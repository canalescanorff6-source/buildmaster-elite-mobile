...
const MAX_MODULES_R178 = 189;
const MAX_SOURCE_BYTES_R178 = 2_690_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R178: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R178: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R178) throw new Error(`R178: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R178}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R178) throw new Error(`R178: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R178}).`);
console.log(`R178 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
