...
const MAX_MODULES_R181 = 182;
const MAX_SOURCE_BYTES_R181 = 2_590_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R181: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R181: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R181) throw new Error(`R181: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R181}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R181) throw new Error(`R181: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R181}).`);
console.log(`R181 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
