...
function convergeR416UniversalPositions(root) {
  ...
  const usage=`import type { AnalysisResult, PositionCode, PositionProficiencyLevel, PositionUsageR416Analysis, PositionUsageR416Entry } from './analyzerDomain';\nimport { POSITION_PT } from './analyzerDomain';import { inspectPlaystyleActivationR124 } from './efootball2027PhaseCatalogR124';\nexport const POSITION_USAGE_R416_VERSION='40.80-r416-universal-position-usage-v1' as const;const ALL:PositionCode[]=['CF','SS','LWF','RWF','LMF','RMF','AMF','CMF','DMF','CB','LB','RB','GK'];\nfunction styleState(off:string,def:string):PositionUsageR416Entry['styleFit']{const active=(x:string)=>x==='LIKELY_ACTIVE'||x==='UNVERIFIED_POSITION',bad=(x:string)=>x==='LIKELY_INACTIVE'||x==='CHECK_POSITION';if(bad(off))return'INACTIVE';if(active(off)&&active(def))return'ACTIVE';if(active(off)||active(def))return'PARTIAL';if(bad(def))return'PARTIAL';return'UNKNOWN';}\nfunction prof(parsed:AnalysisResult['parsed'],p:PositionCode):PositionProficiencyLevel{return p===parsed.mainPosition?'HIGH':parsed.positionProficiencies?.[p]??'UNKNOWN';}\nexport function buildPositionUsageR416(result:AnalysisResult):PositionUsageR416Analysis{const parsed=result.parsed,gk=parsed.mainPosition==='GK',offStyle=parsed.offensivePlaystyle??parsed.playstyle??null,defStyle=parsed.defensivePlaystyle??null;const entries=ALL.map((position):PositionUsageR416Entry=>{const proficiency=prof(parsed,position),off=inspectPlaystyleActivationR124(offStyle,'OFFENSIVE',position),def=inspectPlaystyleActivationR124(defStyle,'DEFENSIVE',position),styleFit=styleState(off.status,def.status),allowed=gk?position==='GK':position!=='GK',natural=position===parsed.mainPosition;let proficiencyState:PositionUsageR416Entry['proficiencyState']=!allowed?'BLOCKED':natural?'NATURAL':proficiency==='HIGH'?'READY':proficiency==='INTERMEDIATE'?'POSITION_TRAINING':proficiency==='LOW'?'LOW':'REVIEW';const styleBlocked=allowed&&!natural&&styleFit==='INACTIVE',recommended=allowed&&!styleBlocked&&(natural||proficiency==='HIGH'),rating=Number(parsed.positionRatings[position]??NaN),ratingScore=Number.isFinite(rating)?Math.max(35,Math.min(100,rating)):60,profScore=natural||proficiency==='HIGH'?100:proficiency==='INTERMEDIATE'?76:proficiency==='LOW'?35:55,styleScore=styleFit==='ACTIVE'?100:styleFit==='PARTIAL'?78:styleFit==='INACTIVE'?30:62,fitScore=Math.round((profScore*.52+styleScore*.3+ratingScore*.18)*10)/10;const reason=!allowed?(gk?'GOL fica travado em GOL; nenhuma adaptação de linha é permitida.':'Jogador de linha não é convertido em GOL.'):styleBlocked?'A posição existe, mas o estilo ofensivo fica fora da lista de ativação; não é recomendada como encaixe ideal.':proficiencyState==='POSITION_TRAINING'?'Proficiência intermediária: a mesma ficha é preservada, mas esta posição deve ser tratada como alvo de Treino de Posição.':proficiencyState==='LOW'?'Proficiência baixa/escura: não recomendar antes de confirmar ou treinar a posição.':proficiencyState==='REVIEW'?'Cor/proficiência não confirmada; a nota isolada não autoriza a posição.':natural?'Posição registrada da carta; ficha canônica permanece a mesma.':'Alta proficiência visual; posição pronta sem recalcular a ficha.';return{position,label:POSITION_PT[position],rating:Number.isFinite(rating)?rating:null,proficiency,proficiencyState,offensiveStyleStatus:off.status,defensiveStyleStatus:def.status,styleFit,recommended,fitScore,reason};});return{version:POSITION_USAGE_R416_VERSION,canonicalBuildLocked:true,sameBuildAcrossFieldPositions:true,goalkeeperLocked:gk,naturalPosition:parsed.mainPosition,entries,recommendedPositions:entries.filter(x=>x.recommended).sort((a,b)=>b.fitScore-a.fitScore).map(x=>x.position),warnings:[gk?'GOL permanece exclusivamente em GOL.':'Todas as posições de linha usam a mesma ficha canônica; posição só altera a avaliação de uso.','Nota numérica e cor/proficiência da grade são sinais independentes.','Estilo cinza/inativo impede recomendação ideal mesmo quando a posição tem nota alta.']};}\nexport function applyPositionUsageR416(result:AnalysisResult):AnalysisResult{return{...result,positionUsageR416:buildPositionUsageR416(result)};}\n`;
  ...
  patch(structuredPath,"  for (const rating of trustedPositionRatingsR133(session, readings)) lines.push(`${rating.code}: ${rating.value}`);","  for (const rating of trustedPositionRatingsR133(session, readings)) lines.push(`${rating.code}: ${rating.value}`);\n  for(const [code,item] of Object.entries(session.detailedReading.positionProficiencies)){if(item&&item.confidence>=68)lines.push(`PROFICIÊNCIA POSICIONAL ${code}: ${item.level}`);}",'R416 structured visual output');
  ...
  const test=`import fs from 'node:fs';import assert from 'node:assert/strict';const read=(p)=>fs.readFileSync(p,'utf8');const domain=read('src/lib/analyzerDomain.ts'),vision=read('src/modules/card-reader/positionProficiencyVisionR416.ts'),reader=read('src/modules/card-reader/readerAnalysisRuntimeR163.ts'),parser=read('src/modules/analysis/analyzerCardEvidenceR186.ts'),engine=read('src/lib/positionUsageIntelligenceR416.ts'),clean=read('src/lib/cleanSlatePerformance2027V4080R119.ts'),pipe=read('src/lib/cardIntelligencePipeline.ts');assert.match(domain,/positionProficiencies\\?: PositionProficiencies/);assert.match(vision,/LWF:\\[\\.02,\\.02,\\.24,\\.29\\]/);assert.match(vision,/INTERMEDIATE/);assert.match(reader,/readPositionProficiencyGridR416/);assert.match(parser,/visualPositions/);assert.match(clean,/parsed\\.mainPosition==='GK'\\|\\|requestedPosition==='GK'/);assert.match(engine,/sameBuildAcrossFieldPositions:true/);assert.match(engine,/styleFit==='INACTIVE'/);assert.match(engine,/proficiency==='INTERMEDIATE'\\?'POSITION_TRAINING'/);assert.match(pipe,/applyPostAuthorityReadOnly\\(current, applyPositionUsageR416\\)/);assert.doesNotMatch(engine,/training\\s*:/);console.log('R416 aprovada: ficha única, GOL bloqueado, cor separada da nota, treino de posição e estilo inativo avaliados sem reescrever progressão.');\n`;
  ...
  for(const [file,fragment,label] of [[cleanPath,"parsed.mainPosition==='GK'||requestedPosition==='GK'",'GOL lock'],[usagePath,'sameBuildAcrossFieldPositions:true','ficha única'],[usagePath,"styleFit==='INACTIVE'",'estilo inativo'],[visionPath,"'INTERMEDIATE'",'proficiência intermediária'],[parserPath,'positionProficiencies,','persistência da grade']]) if(!readFileSync(file,'utf8').includes(fragment)) throw new Error(`R416: contrato ausente: ${label}`);
