...
const MAX_MODULES_R173 = 191;
const MAX_SOURCE_BYTES_R173 = 2_720_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R173: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R173: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R173) throw new Error(`R173: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R173}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R173) throw new Error(`R173: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R173}).`);
console.log(`R173 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
