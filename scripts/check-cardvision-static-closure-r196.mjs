...
const MAX_MODULES_R196 = 178;
const MAX_SOURCE_BYTES_R196 = 2_378_000;
...
for (const [relative, label] of [
  ...
  if (seen.has(path.resolve(path.join(srcRoot, relative)))) throw new Error(`R196: ${label} voltou à árvore estática inicial.`);
}
if (seen.size > MAX_MODULES_R196) throw new Error(`R196: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R196}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R196) throw new Error(`R196: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R196}).`);
console.log(`R196 static closure aprovada: ${seen.size} módulos, ${sourceBytes} bytes; boundaries R188/R190/R191 seguem lazy.`);
