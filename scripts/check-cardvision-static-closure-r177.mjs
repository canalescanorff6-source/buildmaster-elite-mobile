...
const MAX_MODULES_R177 = 191;
const MAX_SOURCE_BYTES_R177 = 2_698_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R177: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R177: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R177) throw new Error(`R177: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R177}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R177) throw new Error(`R177: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R177}).`);
console.log(`R177 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
