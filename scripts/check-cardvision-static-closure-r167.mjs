...
const MAX_MODULES_R167 = 197;
const MAX_SOURCE_BYTES_R167 = 2_830_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R167: ${forbidden} voltou à árvore estática.`);
}
if (seen.size > MAX_MODULES_R167) throw new Error(`R167: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R167}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R167) throw new Error(`R167: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R167}).`);
console.log(`R167 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
