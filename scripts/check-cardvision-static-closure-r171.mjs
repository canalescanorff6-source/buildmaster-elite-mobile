...
const MAX_MODULES_R171 = 191;
const MAX_SOURCE_BYTES_R171 = 2_735_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R171: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R171: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R171) throw new Error(`R171: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R171}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R171) throw new Error(`R171: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R171}).`);
console.log(`R171 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
