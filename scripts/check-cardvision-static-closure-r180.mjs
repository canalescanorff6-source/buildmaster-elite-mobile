...
const MAX_MODULES_R180 = 188;
const MAX_SOURCE_BYTES_R180 = 2_687_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R180: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R180: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R180) throw new Error(`R180: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R180}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R180) throw new Error(`R180: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R180}).`);
console.log(`R180 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
