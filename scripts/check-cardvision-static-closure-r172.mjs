...
const MAX_MODULES_R172 = 191;
const MAX_SOURCE_BYTES_R172 = 2_730_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R172: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R172: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R172) throw new Error(`R172: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R172}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R172) throw new Error(`R172: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R172}).`);
console.log(`R172 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
