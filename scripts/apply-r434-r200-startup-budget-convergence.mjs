...
export const R434_R200_STARTUP_BUDGET_CONVERGENCE_VERSION = '40.80-r434-r200-startup-budget-convergence-v2';
export const R434_R200_STARTUP_BUDGET_BYTES = 675_000;
...
export function patchR200StartupBudgetCheckerR434(source) {
  ...
  throw new Error('R434: contrato de bytes R200 inesperado; nenhum baseline suportado foi encontrado.');
}
...
export function auditR434R200StartupBudget(rootDirectory = process.cwd()) {
  ...
  if (!regression.includes('MAX_SOURCE_BYTES_R200 = 675_000')) issues.push('Regressão R200 não fixa o teto de 675.000 B.');
