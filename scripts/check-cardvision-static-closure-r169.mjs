...
const MAX_MODULES_R169 = 192;
const MAX_SOURCE_BYTES_R169 = 2_765_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R169: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  'src/modules/vault/vaultDeferredRuntimeR169.ts',
  'src/modules/vault/vaultNoteMutationR169.ts',
]) {
  if (!seen.has(path.join(root, required))) throw new Error(`R169: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R169) throw new Error(`R169: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R169}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R169) throw new Error(`R169: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R169}).`);
console.log(`R169 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
