...
const MAX_MODULES_R176 = 191;
const MAX_SOURCE_BYTES_R176 = 2_704_000;
...
for (const forbidden of [
  ...
  if (seen.has(path.join(root, forbidden))) throw new Error(`R176: ${forbidden} voltou à árvore estática.`);
}
for (const required of [
  ...
  if (!seen.has(path.join(root, required))) throw new Error(`R176: contrato leve ausente da árvore estática: ${required}.`);
}
if (seen.size > MAX_MODULES_R176) throw new Error(`R176: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R176}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R176) throw new Error(`R176: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R176}).`);
console.log(`R176 static closure aprovado: ${seen.size} módulos, ${sourceBytes} bytes de fonte estática.`);
