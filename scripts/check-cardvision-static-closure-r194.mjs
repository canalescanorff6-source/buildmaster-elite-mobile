...
const MAX_MODULES_R194 = 178;
const MAX_SOURCE_BYTES_R194 = 2_382_000;
...
for (const [relative, label] of [
  ...
  if (seen.has(path.resolve(path.join(srcRoot, relative)))) throw new Error(`R194: ${label} voltou à árvore estática inicial.`);
}
if (seen.size > MAX_MODULES_R194) throw new Error(`R194: árvore estática cresceu para ${seen.size} módulos (limite ${MAX_MODULES_R194}).`);
if (sourceBytes > MAX_SOURCE_BYTES_R194) throw new Error(`R194: árvore estática cresceu para ${sourceBytes} bytes (limite ${MAX_SOURCE_BYTES_R194}).`);
console.log(`R194 static closure aprovada: ${seen.size} módulos, ${sourceBytes} bytes; fronteiras R188/R190/R191 seguem lazy.`);
