...
check(version === '38.38.0', 'Versão v38.38 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v38.38' && manifest.short_name === 'BuildMaster v38.38', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v38-38-legacy-profile-regressions-1'), 'Cache PWA v38.38 renovado');
...
check(exists('tests/v38-38-legacy-profile-regressions-hotfix.mjs') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3838'), 'Compatibilidade legada v38.38 integrada');
