...
check(version === '34.00.0', 'Versão v34.00 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v34.00' && manifest.short_name === 'BuildMaster v34.00', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v34-00'), 'Cache PWA renovado');
check(!/@import\s+['"]/i.test(css) && !/@import\s+['"]/i.test(executiveCss) && !/@import\s+['"]/i.test(studioCss) && !/@import\s+['"]/i.test(cleanResponsiveCss) && !/@import\s+['"]/i.test(identityCss) && css.includes('.bm-v3000-play-publication') && executiveCss.includes('.bm-v33-sidebar') && studioCss.includes('.bm-v3400-studio') && cleanResponsiveCss.includes('.bm-v3400-clean-responsive') && identityCss.includes('.bm-v3500-identity') && identityCss.includes('visual-midnight-navy'), 'Tema de produção, identidade e temas consolidados');
check(layout.includes('bm-v3000-play-publication') && layout.includes('bm-v3300-executive') && layout.includes('v33-executive.css') && layout.includes('bm-v3400-studio') && layout.includes('v34-studio.css') && layout.includes('bm-v3400-clean-responsive') && layout.includes('v34-clean-responsive.css') && layout.includes('bm-v3500-identity') && layout.includes('v35-identity-themes.css'), 'Escopo visual e identidade ativos');
...
check(String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3200') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3300') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3400') && String(pkg.scripts?.['test:all'] ?? '').endsWith('npm run quality:audit'), 'Bateria atual configurada');
