...
check(version === '38.32.0', 'Versão v38.32 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v38.32' && manifest.short_name === 'BuildMaster v38.32', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v38-32-complete-integration-1'), 'Cache PWA v38.32 renovado');
...
check(exists('src/modules/tactical-studio/metaFormationStudioV3832.ts') && exists('src/modules/tactical-studio/MetaFormationStudioV3832.tsx') && exists('tests/v38-32-complete-integration-regression.mjs'), 'Estúdio Meta e regressão v38.32 integrados');
check(String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3832'), 'Bateria de produção inclui v38.32');
