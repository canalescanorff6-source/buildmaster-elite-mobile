...
check(version === '30.10.0', 'Versão v30.10 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v30.10' && manifest.short_name === 'BuildMaster v30.10', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v30-10'), 'Cache PWA renovado');
...
check(pkg.scripts?.['test:all'] === 'npm run test:v3000 && npm run test:v3010 && npm run quality:audit', 'Bateria atual configurada');
