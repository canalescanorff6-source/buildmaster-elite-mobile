...
check(version === '35.20.0', 'Versão v35.20 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v35.20' && manifest.short_name === 'BuildMaster v35.20', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v35-20'), 'Cache PWA renovado');
check(!/@import\s+['"]/i.test(css) && !/@import\s+['"]/i.test(executiveCss) && !/@import\s+['"]/i.test(studioCss) && !/@import\s+['"]/i.test(cleanResponsiveCss) && !/@import\s+['"]/i.test(identityCss) && !/@import\s+['"]/i.test(solidCss) && css.includes('.bm-v3000-play-publication') && executiveCss.includes('.bm-v33-sidebar') && studioCss.includes('.bm-v3400-studio') && cleanResponsiveCss.includes('.bm-v3400-clean-responsive') && identityCss.includes('.bm-v3500-identity') && identityCss.includes('visual-midnight-navy') && solidCss.includes('.bm-v3520-solid') && solidCss.includes('.bm-dna-profile-grid'), 'Tema de produção, identidade e temas consolidados');
