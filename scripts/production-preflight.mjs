...
check(version === '38.35.0', 'Versão v38.35 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v38.35' && manifest.short_name === 'BuildMaster v38.35', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v38-35-legacy-regressions-1'), 'Cache PWA v38.35 renovado');
...
check(exists('src/modules/tactical-studio/metaFormationStudioV3832.ts') && exists('src/modules/tactical-studio/MetaFormationStudioV3832.tsx') && exists('src/modules/tactical-studio/professionalTacticalTemplateV3833.ts') && exists('tests/v38-33-professional-template-regression.mjs') && exists('tests/v38-34-ci-complete-hotfix-regression.mjs') && exists('tests/v38-35-legacy-regressions-hotfix.mjs'), 'Gerador Tático Profissional e hotfix v38.35 integrados');
check(String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3833') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3834') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3835'), 'Bateria de produção inclui v38.35');
