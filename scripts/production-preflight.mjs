...
check(version === '30.20.0', 'Versão v30.20 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v30.20' && manifest.short_name === 'BuildMaster v30.20', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v30-20'), 'Cache PWA renovado');
...
check(pkg.scripts?.['test:all'] === 'npm run test:v3000 && npm run test:v3010 && npm run test:v3020 && npm run quality:audit', 'Bateria atual configurada');
