...
check(version === '33.00.0', 'Versão v33.00 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v33.00' && manifest.short_name === 'BuildMaster v33.00', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v33-00'), 'Cache PWA renovado');
check(!/@import\s+['"]/i.test(css) && !/@import\s+['"]/i.test(executiveCss) && css.includes('.bm-v3000-play-publication') && executiveCss.includes('.bm-v33-sidebar'), 'Tema de produção e camada Executive consolidados');
check(layout.includes('bm-v3000-play-publication') && layout.includes('bm-v3300-executive') && layout.includes('v33-executive.css'), 'Escopo visual v33 ativo');
...
check(String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3200') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3300') && String(pkg.scripts?.['test:all'] ?? '').endsWith('npm run quality:audit'), 'Bateria atual configurada');
