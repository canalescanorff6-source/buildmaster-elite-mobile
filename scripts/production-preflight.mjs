...
check(version === '38.39.0', 'Versão v38.39 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v38.39' && manifest.short_name === 'BuildMaster v38.39', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v38-39-legacy-contract-final-1'), 'Cache PWA v38.39 renovado');
...
check(exists('tests/v38-38-legacy-profile-regressions-hotfix.mjs') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3838') && exists('tests/v38-39-legacy-contract-final-hotfix.mjs') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3839'), 'Compatibilidade legada v38.38 e correção final v38.39 integradas');
