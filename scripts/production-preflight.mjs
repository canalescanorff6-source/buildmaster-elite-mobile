...
check(version === '38.36.0', 'Versão v38.36 configurada', version);
...
check(manifest.name === 'BuildMaster Elite Tático v38.36' && manifest.short_name === 'BuildMaster v38.36', 'Manifesto PWA sincronizado');
check(sw.includes('buildmaster-v38-36-deterministic-audit-1'), 'Cache PWA v38.36 renovado');
...
check(String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3833') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3834') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3835') && String(pkg.scripts?.['test:all'] ?? '').includes('npm run test:v3836'), 'Bateria de produção inclui v38.36');
