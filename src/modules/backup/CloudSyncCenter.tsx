...
export function CloudSyncCenter(props: {
  ...
  return <section className="bm2900-sync-center">
    ...
    <div className="bm2900-sync-actions">
      <button type="button" onClick={() => void props.onCreateSnapshot()} disabled={props.loading}><History size={19}/><strong>Criar ponto de restauração</strong><span>Salva a versão atual antes de mudanças importantes.</span><small>Local e separado por conta</small></button>
      <button type="button" onClick={() => void props.onSyncFull()} disabled={props.loading || !props.cloudEnabled}>{props.loading ? <Loader2 className="spin" size={19}/> : <CloudUpload size={19}/>}<strong>Sincronizar tudo</strong><span>Envia Cofre, partidas, treinos, formações, imagens e preferências.</span><small>{props.cloudEnabled ? 'Mescla antes de enviar' : 'Supabase necessário'}</small></button>
      <button type="button" onClick={() => void props.onPullMerge()} disabled={props.loading || !props.cloudEnabled}>{props.loading ? <Loader2 className="spin" size={19}/> : <CloudDownload size={19}/>}<strong>Baixar e mesclar</strong><span>Cria uma cópia de segurança e resolve conflitos sem apagar silenciosamente.</span><small>Restauração não destrutiva</small></button>
    </div>
    ...
    <div className="bm2900-snapshot-list">{props.snapshots.map((snapshot) => <article key={snapshot.id}><div><strong>{snapshot.label}</strong><span>{new Date(snapshot.createdAt).toLocaleString('pt-BR')} • {snapshot.deviceLabel}</span><small>{snapshot.recordCount} registros • {snapshot.sections} áreas • {(snapshot.sizeBytes / 1024 / 1024).toFixed(1)} MB • v{snapshot.appVersion}</small></div><div><button type="button" onClick={() => void props.onRestoreSnapshot(snapshot.id)}><RotateCcw size={15}/> Restaurar</button><button type="button" className="danger" aria-label={`Apagar versão ${snapshot.label}`} onClick={() => void props.onDeleteSnapshot(snapshot.id)}><Trash2 size={15}/></button></div></article>)}{!props.snapshots.length && <p className="panel-note">Crie um ponto de restauração antes de testar grandes mudanças.</p>}</div>
  </section>;
}
