...
function extractPlayerName(readings: PremiumZoneReading[], knownPlayerNames: string[] = []): SingleFieldEvidence {
  ...
  const candidates: FieldCandidate[] = clusters.flatMap((cluster) => {
    ...
    const manualMacroReading = cluster.some((item) => ['40.00-calibrated-fields-r1', '40.20-eight-macros-r1', '40.20-nine-macros-r2'].includes(String(item.reading.precisionVersion || '')));
