...
export function matchValidationRevisionR137(records: MatchValidationRecord[]) {
  const signature = records.map((record) => [
    ...
    record.gameVersion ?? '', record.gameplayEpoch ?? '', record.usageFunction ?? '', record.usageContextSignatureR460 ?? '', record.sessionIdR462 ?? '', record.buildGenerationSignatureR464 ?? '',
    JSON.stringify(record.gameplayImpactSnapshotR460 ?? null), JSON.stringify(record.actionRatingsR461 ?? null), JSON.stringify(record.observedMetricKeysR468 ?? null)
  ].join(':')).join('|');
