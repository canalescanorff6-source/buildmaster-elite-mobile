...
import { cardFingerprintAliasesR457, type MatchPerformanceMetrics, type MatchValidationRecord } from '@/lib/appEvolution';
...
function evidenceFingerprintR460(result: AnalysisResult, records: MatchValidationRecord[]) {
  ...
  const rows = [...records].sort((a,b)=>String(a.id).localeCompare(String(b.id))).map((record) => [
    record.id, record.playedAt, record.sessionIdR462 ?? '', record.targetPosition, record.usageFunction ?? '', record.buildSignature,
    ...
    JSON.stringify(record.gameplayImpactSnapshotR460 ?? null), JSON.stringify(record.actionRatingsR461 ?? null), JSON.stringify(record.metrics ?? null)
  ].join(':')).join('|');
...
export function buildBuildOutcomeCalibrationR460(
  ...
  const snapshotRecords = compatible.filter((record) => record.gameplayImpactSnapshotR460?.version === '40.80-r460-build-outcome-snapshot-v1');
  ...
  const status: BuildOutcomeCalibrationR460['status'] = !snapshotRecords.length ? 'NO_EVIDENCE' : active ? 'ACTIVE' : 'OBSERVE';
