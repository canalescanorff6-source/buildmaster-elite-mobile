...
export function CompetitivePerformanceCenter({ formation, teamStyle }: { formation: TacticalFormation; teamStyle: TacticalStyle }) {
  ...
  return <section className="bm2900-performance-center luxury-panel">
    ...
    <nav className="bm2900-tabs"><button type="button" className={tab === 'dashboard' ? 'active' : ''} onClick={() => setTab('dashboard')}><BarChart3 size={16}/> Painel</button><button type="button" className={tab === 'register' ? 'active' : ''} onClick={() => setTab('register')}><Plus size={16}/> Registrar</button><button type="button" className={tab === 'compare' ? 'active' : ''} onClick={() => setTab('compare')}><TrendingUp size={16}/> Comparar</button><button type="button" className={tab === 'report' ? 'active' : ''} onClick={() => setTab('report')}><FileText size={16}/> Relatório</button></nav>
