...
export function AntiDelayCenter({ competitiveMatches }: { competitiveMatches?: CompetitiveMatchRecord[] } = {}) {
  ...
  return <section className="bm2960-anti-delay luxury-panel">
    ...
    {tab === 'preparacao' && <>{diagnosis ? <div className="bm2960-prep-grid">
      ...
      <article className="bm2960-profile"><div className="bm2960-card-title"><Battery size={18}/><strong>Perfil deste aparelho</strong></div><label>Modo gráfico<select value={profile.gameQualityMode} onChange={(event) => saveProfile({ ...profile, gameQualityMode: event.target.value as AntiDelayProfile['gameQualityMode'] })}><option value="desempenho">Desempenho</option><option value="equilibrado">Equilibrado</option><option value="qualidade">Qualidade</option></select></label><label>FPS alvo<select value={profile.targetFps} onChange={(event) => saveProfile({ ...profile, targetFps: Number(event.target.value) as AntiDelayProfile['targetFps'] })}>{[30,60,90,120].map((value) => <option key={value} value={value}>{value} FPS</option>)}</select></label><label>Observações<textarea value={profile.notes} onChange={(event) => saveProfile({ ...profile, notes: event.target.value })}/></label></article>
