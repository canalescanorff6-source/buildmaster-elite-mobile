...
export function applyCalibrationV32(result: AnalysisResult): AnalysisResult {
  ...
  const analysis: CalibrationV32Analysis = {
    ...
    safeguards,
    reasons,
    summary: `A Calibração Automática v40.40 avaliou ${built.reduce((sum, item) => sum + item.candidates, 0) + robust.candidates} candidatas e escolheu um campeão robusto para ${profileLabel(selectedMode).toLowerCase()} com ${champion.score}/100, sem perseguir overall.`
  };
  ...
  const finalResult: AnalysisResult = {
    ...
    buildName: `Ficha Automática v40.40 — ${profileLabel(selectedMode)}`,
