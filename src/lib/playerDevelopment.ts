...
const SPECIAL_RULES: Record<string, { positions: PositionCode[]; attrs: string[]; groups: TrainingKey[]; use: string }> = {
  ...
  'Curva descendente': { positions: ['LWF','RWF','SS','AMF','CF'], attrs: ['Curva','Finalização','Força do chute','Controle de bola'], groups: ['shooting','dribbling','dexterity'], use: 'receber de frente ou cortar para o pé dominante antes da finalização colocada' },
  ...
  'Passador nato': { positions: ['AMF','CMF','DMF','SS','LMF','RMF'], attrs: ['Passe rasteiro','Passe alto','Controle de bola','Condução precisa'], groups: ['passing','dribbling'], use: 'executar passes difíceis sob pressão' },
