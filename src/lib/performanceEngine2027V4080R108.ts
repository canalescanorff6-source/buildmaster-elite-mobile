...
function homemAreaAerialReadyR114(result: AnalysisResult, identity: CanonicalCardIdentityR60) {
  ...
  const heading = attribute(result, 'heading');
  const jump = attribute(result, 'jump');
  const physical = attribute(result, 'physicalContact');
  const aerialBase = (heading + jump + physical) / 3;
  const lowestAerialAttribute = Math.min(heading, jump, physical);
  const ownedAerialSkill = normalize([
    ...(result.parsed.nativeSkills ?? []),
    ...(result.parsed.additionalSkills ?? []),
    ...(result.parsed.specialSkills ?? [])
  ].join(' | '));
  const hasAerialSkillEvidence = /cabec|heading|superioridade aerea|aerial superiority|fortaleza aerea|cabecada fulminante/.test(ownedAerialSkill);
  const dominantAerial = identity.dominantDna.slice(0, 2).includes('aerial');

  // BM_R118_AERIAL_SPECIALIZATION_PROOF:
  // Homem de Área descreve movimentação. Jogo aéreo só vira investimento central
  // quando a própria carta prova ESPECIALIZAÇÃO aérea, não apenas competência.
  const baseQualified = aerialDna >= 88 && aerialBase >= 84 && lowestAerialAttribute >= 78;
  const specializationSignals = [
    dominantAerial,
    hasAerialSkillEvidence,
    aerialDna >= finishingDna + 2,
    aerialDna >= mobilityDna + 2,
    heading >= 88 && jump >= 86
  ].filter(Boolean).length;

  return baseQualified && specializationSignals >= 2;
}
