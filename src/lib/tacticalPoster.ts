...
import { canonicalizePlayerPlaystyle, normalizeFormationCoachStyle } from './efootball2026Playstyles';
import {
...
export type TacticalPosterImageLayerRole = 'watermark' | 'logo' | 'free';

export type TacticalPosterImageLayer = {
  id: string;
...
export type TacticalPosterConfig = {
  ...
  imageLayers?: TacticalPosterImageLayer[];
  instructions: {
    passing: string[];
    recycle: string[];
    attack: string[];
...
function roleForPick(pick: FormationSlotFit, config: TacticalPosterConfig): string {
  ...
  const actualStyle = canonicalizePlayerPlaystyle(pick.player?.parsed.playstyle);
...
function posterImageLayers(config: TacticalPosterConfig, canvasWidth: number, canvasHeight: number, roles: TacticalPosterImageLayerRole[]): string {
  const layers = Array.isArray(config.imageLayers) ? config.imageLayers : [];
  return layers
    .filter((layer) => roles.includes(layer.role) && Boolean(layer.imageDataUrl))
    .slice(0, 8)
    .map((layer) => {
      const x = Math.max(0, Math.min(100, Number(layer.x) || 0));
      const y = Math.max(0, Math.min(100, Number(layer.y) || 0));
      const width = Math.max(4, Math.min(100 - x, Number(layer.width) || 20));
      const height = Math.max(4, Math.min(100 - y, Number(layer.height) || 20));
      const opacity = Math.max(.04, Math.min(1, Number(layer.opacity) || .2));
      return `<image data-layer-role="${layer.role}" href="${escapeXml(layer.imageDataUrl)}" x="${(canvasWidth * x / 100).toFixed(2)}" y="${(canvasHeight * y / 100).toFixed(2)}" width="${(canvasWidth * width / 100).toFixed(2)}" height="${(canvasHeight * height / 100).toFixed(2)}" preserveAspectRatio="xMidYMid meet" opacity="${opacity.toFixed(3)}"/>`;
    })
    .join('');
}
...
export function defaultTacticalPosterInstructions(formation: FormationBlueprint, style: TacticalStyle): TacticalPosterConfig['instructions'] {
  const styleName = styleLabel(normalizeFormationCoachStyle(style));
  return {
...
function createVerticalPosterSvg(config: TacticalPosterConfig): string {
  ...
  const fieldH = 690;
  const coachStyle = styleLabel(normalizeFormationCoachStyle(config.style));
