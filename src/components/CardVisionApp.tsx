...
export function CardVisionApp() {
  ...
  const [sessionSaveState, setSessionSaveState] = useState<'idle' | 'saving' | 'saved' | 'error'>(() => readAccountJsonR141(ACTIVE_SESSION_KEY, null) ? 'saved' : 'idle');
  ...
  useEffect(() => {
    if (mainSection !== 'ajustes') return;
    const retiredViews: CardVisionSettingsView[] = ['evolucao', 'experiencia', 'comunidade', 'comercial'];
    if (retiredViews.includes(settingsView)) {
      setSettingsView('aparencia');
      return;
    }
