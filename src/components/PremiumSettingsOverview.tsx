...
export function PremiumSettingsOverview({ username, version, playerCount, healthScore, cloudEnabled, onOpen }: Props) {
  return (
    <section className="bm32-settings-overview" aria-label="Configurações">
      ...
      <section className="bm32-premium-banner"><span><Crown size={31}/></span><div><h2>Modo Premium <em>ATIVO</em></h2><p>Aproveite todos os recursos exclusivos do Marques Fichas.</p></div><button type="button" onClick={() => onOpen('contas')}>Gerenciar <ChevronRight size={19}/></button></section>
      ...
      <div className="bm32-about-line"><Info size={17}/><span>Marques Fichas • v{version}</span></div>
    </section>
  );
}
