...
} from '../../src/lib/analyzerDomain';
import { ATTRIBUTE_PT, POSITION_PT } from '../../src/lib/analyzerDomain';
import { fitTrainingToExactBudget } from '../../src/modules/builds/trainingOptimizer';
import {
  TRAINING_KEYS,
  emptyTraining,
  trainingPlanCost,
...
import { filterComplementaryAdditionalSkills, skillIdentityKey } from '../../src/lib/officialSkillIdentity';
import { evaluateTrainingPlanProgressionOnlyV3860 } from '../../src/lib/maxMatchPerformanceEngineV3860';
...
export function applyCardFirstAiV3880(result: AnalysisResult): AnalysisResult {
  ...
  const variants = analysis.finalists.slice(0, 3).map((candidate, index) => ({
    ...
    title: index === 0
      ? `Ficha Automática v40.40 — IA por Carta v38.80 — ${analysis.targetFunction}`
